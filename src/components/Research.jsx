...
export function ArticleDialog({
  ...
  return <Modal onClose={onClose} title="ATLANTIS / FELDNOTIZ"><figure className="article-figure"><img src={imagePath(a)} alt={`Illustration zum Thema ${a.title}`} onError={e => {
      ...
      }} /><figcaption>Vorhandene Projektillustration · keine dokumentarische Aufnahme</figcaption></figure><article className="article-body"><div className="article-meta"><span className="tag">{a.category}</span><span>{zone.name} · ca. 2 Min.</span></div><h1 id="dialog-title">{a.title}</h1><p className="article-lead">{a.summary}</p><p>{a.body}</p>{experimentResponse && <section className="experiment-response"><span className="eyebrow">DEIN LOKALER VERSUCH</span><h2>{experimentResponse.title}</h2><p>{experimentResponse.text}</p><SourceLink id={experimentResponse.source}/></section>}<section className="connection-box"><span className="eyebrow">DER ZUSAMMENHANG</span><p>{a.connection}</p></section><h2>Was die Aussage nicht bedeutet</h2><p>{a.limit}</p><h2>Was wir daraus mitnehmen</h2><p>{a.action}</p><small className="editorial-note">Redaktionelle Handlungseinordnung auf Basis der folgenden Quellen.</small><section className="article-sources"><h2>Nachlesen & überprüfen</h2>{a.sources.map((id, i) => <a href={SOURCES[id].url} target="_blank" rel="noopener noreferrer" key={id}><span className="source-number">0{i + 1}</span><span><strong>{SOURCES[id].org}</strong>{SOURCES[id].title}<small>{SOURCES[id].year}</small></span><ExternalLink size={17} /></a>)}<small>Redaktionell geprüft: {REVIEW_DATE}. Keine Live-Daten.</small></section><button className="primary-button" onClick={() => onZone(a.zone)}><MapPin size={17} /> Diese Zone erkunden <ArrowRight size={17} /></button><section className="related"><h2>Weiterdenken</h2>{related.map(item => <button key={item.id} onClick={() => onOpen(item.id)}>{item.title}<ArrowUpRight size={17} /></button>)}</section></article></Modal>;
}
