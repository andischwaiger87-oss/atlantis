...
export default function App() {
  ...
  return <div className={`app-shell remake ${view === 'expedition' ? 'immersive' : 'reading-view'}`}><a className="skip-link" href="#main">Zum Inhalt</a><header className="topbar"><a className="brand" href="#" aria-label="Atlantis – zum Horizont" onClick={e => {
      ...
      }}>ATLANTIS<small className="brand-version">v.2</small></a><nav aria-label="Hauptnavigation">{[['expedition', 'Expedition', <Compass size={16} />], ['knowledge', 'Wissen', <BookOpen size={16} />], ['climate', 'Klimalabor', <SlidersHorizontal size={16} />]].map(([id, label, icon]) => <button key={id} className={view === id ? 'active' : ''} aria-current={view === id ? 'page' : undefined} onClick={() => changeView(id)}>{icon}<span>{label}</span></button>)}</nav><button className="about-button" aria-label="Quellen und Methodik" onClick={() => setMethod(true)}><Info size={18} /></button></header>
